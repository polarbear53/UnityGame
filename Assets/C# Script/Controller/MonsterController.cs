...
public class MonsterController : MonoBehaviour
{
    ...
    void Update()
    {
        if (currHp <= 0)
        {
            ...
            exp.GetComponent<EXPController>().exppoint = 10 * dmg;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("gi")) //gi 태그를 가진 오브젝트와 부딪히면
        {
            ...
            if (currHp > 0)
            { //현재 체력이 남아있다면
                if (collision.gameObject.name == "giPrefab(Clone)")
                {
                    Debug.Log("player -> -" + GameObject.Find("player").GetComponent<PlayerController>().giDamage);
                    currHp -= GameObject.Find("player").GetComponent<PlayerController>().giDamage; //현재 체력 갂기
                }
                else
                {
                    Debug.Log("tower -> -" + GameObject.Find("tower").GetComponent<TowerController>().towerdamage);
